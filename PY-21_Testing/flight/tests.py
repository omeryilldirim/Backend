...
from .views import FlightView, ReservationView, Flight
...
class FlightTestCase(APITestCase):
    ...
    def test_flight_create_as_admin_user(self):
        data = {
            ...
            "departure_city": "Antalya",
            "arrival_city": "Amsterdam",
            "date_of_departure": "2023-12-03",
            "etd": "15:27:22"
        }
        request = self.factory.post(reverse('flight-list'), data)
        force_authenticate(request, user=self.user)
    ...
    def test_flight_update_as_admin_user(self):
        ...
        request = self.factory.put(reverse('flight-detail', kwargs={'pk': 1}), data)
        force_authenticate(request, user=self.user)
        self.user.is_staff = True
        self.user.save()
